...
A1. Carroll County had the highest number of calls with 448, followed by Cecil County with 438, and Allegany County with 418.
...
Both of the top two results were August, and there seems to be more calls in the summer months in general.

Then calculate the most calls per county and location. Which location had the most calls - where and what is that location?

314 Grove Neck Rd had the most calls with 15. It is an addiction and treatment center in Earleville, Md.
...
A3.The zip code 21217 has the highest rate of calls per 1,000 people, with 20.9. There is a very high population of people under 18 in this zip code (81.5%) and a very high population of black non-hispanic (86.4%).
...
A4: Write code to isolate calls in zip codes where the population over 65 is at least 15%. Of those zip codes, which has the highest number of calls per 1000 people?

21215 has the highest calls per 1000 people (8.3) of zip codes with a population over 65 of at least 15%.

```{r}
baltimore_911_with_demographics |>
  filter(pct_pop_65 > 14.9) |>
  group_by(zip_code, population) |>
  summarise(total_calls = n()) |>
  mutate(calls_per_1000 = total_calls/population*1000) |>
  arrange(desc(calls_per_1000))
