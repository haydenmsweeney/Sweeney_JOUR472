...
cleaned_md_grants_loans <- md_grants_loans <- read_csv("data/State_of_Maryland_Grant_and_Loan_Data__FY2009_to_FY2022.csv" , guess_max=373564) |> clean_names()
```
...
```{r}
pg_schools <- payments  |>
  filter((str_detect(vendor_name, 'PRINCE GEORGES SCHOOL')) |
  str_detect(vendor_name, 'PG COUNTY PUBLIC') | str_detect(vendor_name, 'PRINCE GEORGES COUNTY PUBLIC') | str_detect(vendor_name, 'PG COUNTY SCHOOL'))
```
...
A2: There are only five years provided in the dataset where there are payments towards PG schools. I wonder if that data just wasn't made available for the other years. I also notice that the total amount in 2021 was significantly higher than the amount for the other years.
...
A3. 1. St. Joseph Orthopaedics LLC 
  2. St Joseph Medical Group
  3. St Joseph Medical Center
  All of these should be combined into one. Here, you would use case_when and str_detect and change it to one universal name when R detects any of those three names.

```{r}
payments  |>
  filter((str_detect(vendor_name, 'UNIVERSITY OF MARYLAND')) |
  str_detect(vendor_name, 'UMD') | str_detect(vendor_name, 'U OF MD')) |>
...
A4: Stem cell research funding has generally decreased over time, with 2009 having the highest amount of funding in this dataset, and 2021 having the lowest.

```{r}
cleaned_md_grants_loans <- md_grants_loans |>
  filter ((str_detect(description, 'STEM CELL')) | str_detect(description, 'Stem Cell') | str_detect(description, 'stem cell')) |>
