...
md_statewide_county <- read_csv("data/md_statewide_county.csv") |>
  ...
  mutate(total_gov = moore + cox + lashar + wallace + harding + sefcik + gov_writein) |>
...
**A2.** There are several outliers here - outages are extremely high in Talbot County and very low in Frederick and Washington County. My next step would be to see if this data is consistent across all months of the year, and why outages are higher in some counties - weather?
...
outages_with_geometry <- county_outages_march2023 |>
  left_join(md_counties, join_by(county == "NAME"))
```

```{r}
#Map it out
ggplot() +
  geom_sf(data = outages_with_geometry, aes(fill = outages_per_10000)) +
  theme_minimal() +
  scale_fill_gradient2()
```
...
electric_vehicles_county <- read_csv("data/ev_2020_2023.csv") |>
mutate(pct_change = (((march_2023 - july_2020)/july_2020))*100)
```

```{r}
#Merge geometries with EV data
electric_vehicles_with_geometry <- electric_vehicles_county |>
  left_join(md_counties, join_by(county == "NAME"))
```
...
#Map out the change
ggplot() +
  geom_sf(data = electric_vehicles_with_geometry, aes(fill = pct_change)) +
  theme_minimal() +
  scale_fill_gradient2()
```
...
electric_vehicles_with_geometry_population <- electric_vehicles_with_geometry |>
