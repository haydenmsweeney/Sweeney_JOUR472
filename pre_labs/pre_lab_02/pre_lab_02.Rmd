...
**Answer** If we only want to work with these two columns, now we can call the new variable to give us just these columns.
...
**Answer:** The <chr> for that column changed to <date>, meaning R now recognizes it has a date.
...
**Answer** R is still recognizing that column as a date, so it's way of representing the month is by the date of the first day of each month.
...
**Answer** It is telling R to add up the total number of votes for all candidates, and then to calculate the percentage of those votes that were for Wes Moore.
...
**Answer** It has now created a variable for total votes, making it quicker to calculate the percentage of votes for Moore, and it is now representing that as a percentage. I think* the arrange portion of this code is putting the percentage of votes for Moore in each county in descending order.
...
**Answer** I don't think so, because this data is better represented as percentages rather than a bunch of big numbers.
...
**Answer** I asked ChatGPT: "What are two other tidyverse functions similar to str_to_upper?"
It said that toupper() and 'stringr' both do the same function, which is converting strings to uppercase.
...
**Answer** I think the mutate statement is telling R that when they see each of these states in the address, to represent the address with the corresponding abbreviation for that state.
...
**Answer** Yes, I believe it is showing the total number of expenses for each state. It would be more helpful if the numbers in the "total" column had dollar signs.
...
1. What Maryland county had the lowest percentage of votes for Wes Moore?

2. What Maryland county had the highest percentage of votes for Cox?

3. What payee made the highest amount of contributions?
