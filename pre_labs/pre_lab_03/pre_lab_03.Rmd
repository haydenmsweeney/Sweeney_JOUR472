...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** Yes, there is an error message that says "Use `spec()` to retrieve the full column specification for this data"
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** The "Vendor Name" field is <chr>, the "Amount" field is <dbl>, and the "Date" columb is <chr>.
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer** One entry, which I would assume is meant to be "Loan" is entered as "L". 
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer** The smallest amount is $60 from the Department of Natural Resources/Rural Legacy Program.
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer** The header names are now all lowercase.
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** I'm not sure, but I think there are now duplicate zip codes.
...
The zip code 21601 has gotten the most amount of money from the Maryland Tourism Board, with $800,000. Yes, this zip code makes sense.

```{r}
cleaned_md_grants_loans |>
  group_by (zip5) |>
  filter (str_detect (source, 'COMMERCE/MARYLAND TOURISM BOARD')) |>
  arrange (desc(amount))
```
