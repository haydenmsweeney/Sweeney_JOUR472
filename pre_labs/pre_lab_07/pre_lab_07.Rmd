...
```{r echo=FALSE}
census_api_key("830688142e6cbfa41054e8273fa8e30c8956b099", install=TRUE)
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer** Nevada had the largest percentage change from 2000 to 2010, with 35.1%.
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer** Howard County has the highest median household income ($129,549).
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer** Margin of error for household income by county in Maryland
...
Is the difference between Calvert and Montgomery significant? **Answer** No, because the margin of error bars suggest that Montgomery County's median household income actually could be higher than Calvert's.

Is the difference between Howard and everyone else significant? **Answer** Yes because all of the other counties are outside Howard County's margin of error bar, so we can say with certainty that Howard County has the highest median household income in Maryland.
...
**Task** Let's look at the average household income for high schools that have 60% or more graduates immediately enrolling in college, and then look at those with less than 40%. Write a sentence describing the results. **Answer** There are 76 high schools in Maryland with either 60% or more or less than 40% of graduates immediately enrolled in college, and those schools have an average household income of $69,252.24.
...
**Task** What is another question you could ask of this data? **Answer** What is the average household income in high schools with between 40% and 60% of graduates immediately enrolled in college?
