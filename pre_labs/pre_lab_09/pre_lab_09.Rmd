...
https://datawrapper.dwcdn.net/ChmSv/1/
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **Answer** Describe the changes from the previous map: The map is only Maryland and only includes counties that actually have data.
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "0.0%")}} **Answer** Describe the changes from the previous map: The map looks a lot cleaner; you can hover over each county and see the name of the county as well as the percentage of votes to one decimal point.
...
https://datawrapper.dwcdn.net/PkjKB/1/
