...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** Some of the same grantees are typed differently (ex: WASHINGTON CO COMMISSIONE, WASHINGTON CO COMMISSIONERS)
...
1. Changed "Charles County Arts Alliance Inc" to "CHARLES COUNTY ARTS ALLIANCE"
2. Changed "FAIR HILL FARM, INC." to "FAIR HILL FARM INC"
...
1. Changed "WASHINGTON ADVENTIST UNIVERSITY" to "Washington Adventist Universtiy" - this is the only change I found.
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** No, it looks the same.
